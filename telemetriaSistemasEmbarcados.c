...
typedef struct {
    ...
    uint32_t status    :  3;  // bits 10-12: codigo de status (0-7)       
    uint32_t bateria   :  1;  // bit  13   : bateria baixa (0=ok, 1=crit) 
    int32_t  temp      : 12;  // bits 14-25: temperatura (complemento 2)  
...
int main(void) {
    ...
    // Leitura de hexadecimais ate receber 0 ou 0x0 
    while (scanf("%x", &entrada) == 1 && entrada != 0) {

        // Dobra a capacidade quando o vetor esta cheio 
    ...
    // Processamento e exibicao do relatorio 
